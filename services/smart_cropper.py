...
class SmartCropper:
    ...
    def _track_person(self, clip: VideoFileClip, crop_width: int,
        ...
        if best_face is not None:
            ...
            crop_x = max(
                0,
                min(face_center_x - crop_width // 2, video_width - crop_width))
            crop_y = max(
                0,
            ...
            logger.info(
                f"Face detected: {w}x{h}px at ({x}, {y}), using crop position ({crop_x}, {crop_y})"
            )
        else:
            ...
            logger.info(
                f"No face detected, using center crop at ({crop_x}, {crop_y})")
    ...
    def _track_mouse_or_feature(
        ...
        if activity_zones:
            ...
            if total_weight > 0:
                ...
                logger.info(
                    f"Activity detected at ({int(weighted_x)}, {int(weighted_y)}), using crop position ({crop_x}, {crop_y})"
                )
            else:
                ...
                crop_y = (video_height - crop_height) // 2
                logger.info(
                    f"Low activity detected, using center crop at ({crop_x}, {crop_y})"
                )
        else:
            # Fallback to center crop
            crop_x = (video_width - crop_width) // 2
            crop_y = (video_height - crop_height) // 2
            logger.info(
                f"Analysis failed, using center crop at ({crop_x}, {crop_y})")
