...
# ============================================
# LOGO WATERMARK ENDPOINTS
# ============================================

class AddLogoRequest(BaseModel):
    """Request model for adding logo to clip"""
    logo_data_url: str  # Base64 encoded image data URL
    position: Optional[str] = "top_right"  # top_right, top_left, bottom_right, bottom_left
    opacity: Optional[float] = 0.9
    padding: Optional[int] = 20
    logo_size: Optional[int] = 80


@app.post("/api/v1/clips/{clip_id}/add-logo")
async def add_logo_to_clip(
    clip_id: int,
    request: AddLogoRequest,
    background_tasks: BackgroundTasks
):
    """
    Add brand logo watermark to video clip
    
    Args:
        clip_id: ID of the clip
        request: Logo configuration (base64 image, position, opacity, etc.)
    ...
    try:
        # Get clip from database
        db = next(get_db())
        clip = db.query(Short).filter(Short.id == clip_id).first()
        ...
        logger.info(f"Started logo addition job {job_id} for clip {clip_id}")
        
        return {
            "job_id": job_id,
            "status": "processing",
            "message": "Adding logo to video..."
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to start logo job: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


async def add_logo_task(
    job_id: str,
    clip_id: int,
    video_path: str,
    logo_data_url: str,
    ...
    """
    Background task to add logo watermark to video
    
    Args:
        job_id: Unique job identifier
        clip_id: Clip database ID
        video_path: Path to input video
        logo_data_url: Base64 encoded logo image
        position: Logo position on video
        opacity: Logo opacity (0.0-1.0)
        padding: Padding from edge in pixels
        logo_size: Logo display size in pixels
    """
    try:
        logger.info(f"Logo task started: {job_id}")
        
        await progress_tracker.update_progress(
            job_id,
            "processing",
            10,
            "Initializing logo watermarker..."
        )
        ...
        output_path = str(video_path_obj.parent / f"{video_path_obj.stem}_with_logo{video_path_obj.suffix}")
        ...
        # Add logo to video
        result_path = watermarker.add_logo_to_video(
            video_path=video_path,
        ...
        # Update clip in database with new file path
        db = next(get_db())
        clip = db.query(Short).filter(Short.id == clip_id).first()
        if clip:
            clip.file_path = result_path
            db.commit()
            logger.info(f"Updated clip {clip_id} with logo video: {result_path}")
        ...
        logger.info(f"Logo task completed: {job_id}")
        
    except RuntimeError as e:
        logger.error(f"Logo task failed: {str(e)}")
        await progress_tracker.update_progress(
            job_id,
            "failed",
            0,
            f"Failed to add logo: {str(e)}"
        )
    except Exception as e:
        logger.error(f"Unexpected error in logo task: {str(e)}")
        await progress_tracker.update_progress(
            job_id,
            "failed",
            0,
